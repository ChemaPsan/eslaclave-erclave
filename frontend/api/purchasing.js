...
export async function getPurchasingWorkspace(){const names=["suppliers","requisitions","orders","receipts"];const values=await Promise.allSettled(names.map(name=>request(`/v1/purchasing/${name}`)));const result={suppliers:[],requisitions:[],orders:[],receipts:[],errors:{}};values.forEach((value,index)=>value.status==="fulfilled"?result[names[index]]=value.value.data:result.errors[names[index]]=value.reason);return result;}
