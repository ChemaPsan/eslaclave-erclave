...
`identity-plan.json` reemplaza la cuenta de computo predeterminada por identidades separadas para Admin, Produccion, Inventory, RH, Ventas, Compras, Mantenimiento, migraciones y GitHub Actions. La autenticacion del pipeline usa Workload Identity Federation; no se almacenan llaves JSON.
...
Estado comprobado el 2026-08-23: las siete cuentas originales, Artifact Registry `erclave-qa` y el provider WIF limitado a `ChemaPsan/eslaclave-erclave` estan aprovisionados. Las identidades nuevas `erclave-purchasing-qa` y `erclave-maintenance-qa` estan declaradas, pero deben aprovisionarse y verificarse mediante una actividad `qa-write` independiente antes del candidato de siete servicios. Admin runtime conserva `roles/firebaseauth.admin` porque Backoffice crea, actualiza y elimina identidades Firebase durante el ciclo de vida de tenants; no concede autorizacion ERClave ni acceso a datos operativos. La identidad desplegadora cuenta con `roles/firebasehosting.admin` para publicar exclusivamente mediante el gate `qa-frontend`. No se permiten llaves JSON del pipeline.
...
| `QA_SALES_RUNTIME_SERVICE_ACCOUNT` | Correo de `erclave-sales-qa` |
| `QA_PURCHASING_RUNTIME_SERVICE_ACCOUNT` | Correo de `erclave-purchasing-qa` |
| `QA_MAINTENANCE_RUNTIME_SERVICE_ACCOUNT` | Correo de `erclave-maintenance-qa` |
...
| `QA_PURCHASING_API_URL` | URL HTTPS estable de Compras QA |
| `QA_MAINTENANCE_API_URL` | URL HTTPS estable de Mantenimiento QA |
...
1. `qa-candidate.yml` exige SHA completo, validacion local equivalente y confirmacion `BUILD_ERCLAVE_QA`; construye siete imagenes y registra digests.
...
4. El job estructural idempotente sincroniza permisos y deja activos solamente `admin`, `production`, `inventory`, `hr`, `sales`, `purchasing` y `maintenance`; no carga almacenes, articulos, movimientos, areas, puestos, recetas, ordenes ni documentos comerciales.
...
7. `qa-traffic` requiere otra aprobacion. Antes de escribir, resuelve desde JSON exactamente una revision `candidate` para cada uno de los siete servicios; solo con el conjunto completo mueve el trafico y verifica que cada revision certificada reciba 100%.
