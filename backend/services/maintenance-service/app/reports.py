...
def build_report(repository,tenant_id,report_code,filters):
    ...
    if report_code=="orders":
        ...
        rows=_query(repository,f"""select o.code order_code,x.warehouse_name_snapshot warehouse_name,x.status request_status,l.line_number,l.item_code_snapshot item_code,l.item_name_snapshot item_name,l.quantity,coalesce((select sum(a.quantity) from maintenance.material_return_adjustments a where a.tenant_id=l.tenant_id and a.resource_id=l.id),0) returned_quantity,l.unit_code,l.unit_cost_snapshot,(l.quantity*coalesce(l.unit_cost_snapshot,0)-coalesce((select sum(a.quantity*a.unit_cost) from maintenance.material_return_adjustments a where a.tenant_id=l.tenant_id and a.resource_id=l.id),0)) line_cost,l.line_status,x.created_at
        ...
        columns=[C("order_code","Orden","Order"),C("warehouse_name","Almacén","Warehouse"),C("request_status","Estatus de solicitud","Request status"),C("line_number","Partida","Line"),C("item_code","Refacción","Spare part"),C("item_name","Nombre","Name"),C("quantity","Cantidad entregada","Issued quantity"),C("returned_quantity","Cantidad devuelta","Returned quantity"),C("unit_code","Unidad","Unit"),C("unit_cost_snapshot","Costo unitario","Unit cost"),C("line_cost","Costo total","Total cost"),C("line_status","Estatus de partida","Line status"),C("created_at","Solicitada","Requested")]
    else:
