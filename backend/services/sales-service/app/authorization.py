...
class AuthorizedContext:
    ...
    def require(self, permission: str) -> None:
        ...
        if permission not in allowed:
            raise ErclaveError("permission_denied", "Required Sales permission is missing.", status_code=403, details={"permission": permission})
...
def require_sales_access(permission: str | tuple[str, ...]):
    def dependency(
        ...
        if settings.auth_mode != "firebase":
            return AuthorizedContext(x_tenant_id, x_actor_id or "usr_demo", required[0], frozenset(required))
        ...
        return AuthorizedContext(x_tenant_id, actor_id, required[0], frozenset(context.get("permissions", [])))

    return dependency


def require_service_order_transition_access(
    action: str,
    x_tenant_id: str | None = Header(default=None, alias="X-Tenant-Id"),
    authorization: str | None = Header(default=None, alias="Authorization"),
    x_actor_id: str | None = Header(default=None, alias="X-Actor-Id"),
    ...
    allowed_actions = {"assign", "start", "wait", "resume", "submit_acceptance", "accept", "cancel"}
    if action not in allowed_actions:
        raise ErclaveError("service_order_action_invalid", "Service order action is not supported.", status_code=422)
    permission = f"sales.service_order.{action}"
    if not x_tenant_id:
        raise ErclaveError("tenant_required", "X-Tenant-Id header is required.", status_code=400)
    if settings.auth_mode != "firebase":
        return AuthorizedContext(x_tenant_id, x_actor_id or "usr_demo", permission)
    if not authorization or not authorization.lower().startswith("bearer "):
        raise ErclaveError("auth_required", "Authorization Bearer token is required.", status_code=401)
    context = client.get_context(x_tenant_id, authorization)
    tenant = context.get("tenant") or {}
    ...
    if not actor_id:
        raise ErclaveError("authorization_service_unavailable", "Authorization context is incomplete.", status_code=503)
    return AuthorizedContext(x_tenant_id, actor_id, permission)
