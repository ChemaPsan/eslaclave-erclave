...
Primer corte operativo en Local (`implemented`). El runtime FastAPI escucha en `http://127.0.0.1:8010` y persiste en el schema tenant-safe `purchasing`. Compras puede activarse para un tenant de servicios sin habilitar Inventory; las partidas inventariables conservan Inventory como autoridad condicional. QA y Produccion permanecen sin despliegue de este cambio.
...
- recepcion comercial parcial/total multipardida de articulos y servicios, con reclamo transaccional del saldo abierto;
- reconciliacion manual idempotente que reintenta solo partidas pendientes contra `inventory-service`.

Admin permanece como autoridad transversal de sesion, permisos, unidades, monedas, condiciones de pago y folios. Inventory es una dependencia condicional exclusiva de partidas `inventory_item`; una partida `service` prohíbe articulo y almacen, y su recepcion comercial no invoca Inventory.
...
Las recepciones inventariables consumen `POST /v1/inventory/purchase-receipts`, una frontera dedicada que solo admite entradas con origen `purchase_order` y partida explicita. El receptor necesita `purchasing.receipt.create`, no autoridad para registrar movimientos manuales arbitrarios. Las recepciones de servicio solo completan el saldo comercial en Compras y dejan el movimiento de Inventory en `null`.
