...
class FakeRepo:
    ...
    def list_roles(self,t,area=None,production_only=False,maintenance_only=False):return [schemas.RoleRead(id="hrp_1",labor_area_id="hra_1",position="Operador",recipe_name="Operador",resource_quantity=2,minutes_per_resource=480,hourly_cost=120,intervenes_in_production=True,intervenes_in_maintenance=True,status="active")] if t==TENANT else []
    ...
    def list_workers(self,t,position=None,production_only=False,active_only=False,maintenance_only=False):
        return [schemas.WorkerRead(id="hrw_1",employee_number="EMP-1",first_names="Ana",first_last_name="Garcia",curp="GARC900101HDFRRL09",rfc="GARC900101ABC",nss="12345678903",hire_date=date(2026,1,1),labor_position_id="hrp_1",status="active",full_name="Ana Garcia",position_name="Operador",labor_area_id="hra_1",labor_area_name="Operaciones",intervenes_in_production=True,intervenes_in_maintenance=True)] if t==TENANT else []
...
def test_maintenance_projection_filters_server_side():
    response=client().get("/v1/hr/workers/maintenance-eligible",headers=headers());assert response.status_code==200;assert response.json()["data"][0]["intervenes_in_maintenance"] is True
