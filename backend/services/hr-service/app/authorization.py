...
class AuthorizedContext:
    tenant_id:str;actor_id:str;permission:str;permissions:frozenset[str]=frozenset()
    def require(self,permission):
        allowed=self.permissions or frozenset({self.permission})
        if permission not in allowed:raise ErclaveError("permission_denied","Required HR permission is missing.",status_code=403,details={"permission":permission})
class AdminSessionClient:
    ...
    def get_context(self,tenant_id,authorization):
        ...
        except error.HTTPError as exc:raise ErclaveError("authorization_denied","HR authorization failed.",status_code=401 if exc.code==401 else 403) from exc
        except (error.URLError,TimeoutError,KeyError,ValueError) as exc:raise ErclaveError("authorization_service_unavailable","Authorization service is unavailable.",status_code=503) from exc
def get_admin_session_client(settings:Settings=Depends(get_settings)):return AdminSessionClient(settings)
def require_hr_access(permission):
    def dependency(x_tenant_id:str|None=Header(None,alias="X-Tenant-Id"),authorization:str|None=Header(None,alias="Authorization"),x_actor_id:str|None=Header(None,alias="X-Actor-Id"),settings:Settings=Depends(get_settings),client:AdminSessionClient=Depends(get_admin_session_client)):
        ...
        if "hr" not in context.get("active_modules",[]):raise ErclaveError("module_not_enabled","HR module is not enabled.",status_code=403)
        if not any(item in context.get("permissions",[]) for item in required):raise ErclaveError("permission_denied","Required HR permission is missing.",status_code=403,details={"permission":required[0]} if len(required)==1 else {"permissions_any":list(required)})
